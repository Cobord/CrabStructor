    ...
    if let Data::Struct(data_struct) = &ast.data {
        match &data_struct.fields {
            Fields::Named(fields_named) => {
                ...
                let mut default_of_type : std::collections::HashSet<String> = std::collections::HashSet::new();
                            ...
                            for nested_meta in meta_list.nested {
                                match nested_meta {
                                    ...
                                    NestedMeta::Meta(Meta::Path(nv)) => {
                                        for this_field in nv.get_ident().unwrap().to_string().split(',') {
                ...
                for field in &fields_named.named {
                    ...
                    let use_default_of_field_type = default_of_type.contains(&field_name.as_ref().unwrap().to_string());
